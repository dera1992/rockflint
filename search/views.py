...
def filter(request):
    ...
    if order == "property_price":
        qs = qs.order_by(order)
    elif order == "-property_price":
        qs = qs.order_by(order)
    ...
    context = {
        'queryset': queryset,'categories': categories,'states':states,'cities':cities,'offers':offers,'is_favourite': is_favourite,
        'order': order,'page': page,'air_conditioning':air_conditioning,'parking':parking,'sewer':sewer,'water':water,'lawn':lawn,
        'swimming_pool':swimming_pool,'barbecue':barbecue,'tv_cable':tv_cable,'microwave':microwave,'wi_fi':wi_fi,'category':category,
        'state':state,'city':city,'bedroom':bedroom,'bathroom':bathroom,'gym':gym,'church':church,'mosque':mosque,'beach':beach,
        'school':school,'market':market,'hospital':hospital,'resturant':resturant
...
#
#     if request.is_ajax():
#         html = render_to_string('search/ajax_search.html',request=request)
